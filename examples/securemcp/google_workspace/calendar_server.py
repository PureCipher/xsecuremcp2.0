"""PureCipher Calendar: OAuth + SecureMCP policy, consent, provenance and gating."""
...
TOOLS = frozenset({"calendar_list_events", "calendar_list_calendars"})
...
class CalendarReadPolicy:
    async def evaluate(self, context: PolicyEvaluationContext) -> PolicyResult:
        ...
        return PolicyResult(
            ...
            reason="Only the declared Calendar read tools are permitted.",
            policy_id="purecipher-calendar-readonly-v1",
        )

    async def get_policy_id(self) -> str:
        return "purecipher-calendar-readonly-v1"

    async def get_policy_version(self) -> str:
        return "1.0.0"


async def calendar_get(path: str, params: dict | None = None) -> dict:
    token = get_access_token()
    if token is None or CALENDAR_SCOPE not in token.scopes:
        raise ValueError("Google authorization with calendar.readonly is required.")
    ...
    if response.status_code >= 300:
        ...
        raise ValueError(f"Calendar request failed ({response.status_code}).")
...
def create_server(client_id: str, client_secret: str, base_url: str) -> SecureMCP:
    ...
    server = SecureMCP(
        ...
        security=SecurityConfig(
            policy=PolicyConfig(providers=[CalendarReadPolicy()], fail_closed=True),
            consent=ConsentConfig(
                graph_id="purecipher-calendar", resource_owner="purecipher"
            ),
            provenance=ProvenanceConfig(ledger_id="purecipher-calendar"),
    ...
    async def calendar_list_calendars(
        ...
        return await calendar_get(
            "users/me/calendarList", page_params(page_token, max_results)
        )
    ...
    async def calendar_list_events(
        ...
        """List one page of events in an accessible calendar."""
        return await calendar_get(
            "calendars/" + safe_id(calendar_id) + "/events",
...
if __name__ == "__main__":
    create_server(
        ...
        os.environ.get("PURECIPHER_CALENDAR_BASE_URL", "http://127.0.0.1:9104"),
    ).run(transport="http", host="127.0.0.1", port=9104)
