"""PureCipher Tasks: OAuth + SecureMCP policy, consent, provenance and gating."""
...
TOOLS = frozenset({"tasks_list_tasklists", "tasks_list_tasks"})
TASKS_SCOPE = "https://www.googleapis.com/auth/tasks.readonly"


class TasksReadPolicy:
    async def evaluate(self, context: PolicyEvaluationContext) -> PolicyResult:
        ...
        return PolicyResult(
            ...
            reason="Only the declared Tasks read tools are permitted.",
            policy_id="purecipher-tasks-readonly-v1",
        )

    async def get_policy_id(self) -> str:
        return "purecipher-tasks-readonly-v1"

    async def get_policy_version(self) -> str:
        return "1.0.0"


async def tasks_get(path: str, params: dict | None = None) -> dict:
    token = get_access_token()
    if token is None or TASKS_SCOPE not in token.scopes:
        raise ValueError("Google authorization with tasks.readonly is required.")
    ...
    if response.status_code >= 300:
        ...
        raise ValueError(f"Tasks request failed ({response.status_code}).")
...
def create_server(client_id: str, client_secret: str, base_url: str) -> SecureMCP:
    ...
    server = SecureMCP(
        ...
        security=SecurityConfig(
            policy=PolicyConfig(providers=[TasksReadPolicy()], fail_closed=True),
            consent=ConsentConfig(
                graph_id="purecipher-tasks", resource_owner="purecipher"
            ),
            provenance=ProvenanceConfig(ledger_id="purecipher-tasks"),
    ...
    async def tasks_list_tasklists(page_token: str = "", max_results: int = 20) -> dict:
        ...
        return await tasks_get("users/@me/lists", page_params(page_token, max_results))
    ...
    async def tasks_list_tasks(
        ...
        """List tasks in a task list accessible to the authenticated user."""
        return await tasks_get(
            "lists/" + safe_id(tasklist_id) + "/tasks",
            page_params(page_token, max_results),
        )
...
if __name__ == "__main__":
    create_server(
        ...
        os.environ.get("PURECIPHER_TASKS_BASE_URL", "http://127.0.0.1:9103"),
    ).run(transport="http", host="127.0.0.1", port=9103)
