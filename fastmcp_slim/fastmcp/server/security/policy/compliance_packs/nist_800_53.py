...
_AC_2 = ComplianceRuleSpec(
    ...
    deny_message=(
        "NIST AC-2: Access requires an account_id and a permitted account_type."
    ),
    citation=_cite("AC-2"),
)
...
_AC_3 = ComplianceRuleSpec(
    ...
    tags=frozenset({"nist_moderate", "information_system_access"}),
    checks=(MetadataCheck(metadata_key="authorization_decision_id"),),
    deny_message=(
        "NIST AC-3: Access requires an authorization_decision_id from the PDP."
    ),
    citation=_cite("AC-3"),
)
...
_AC_6 = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="granted_scope"),),
    deny_message=("NIST AC-6: Privileged functions require a granted_scope."),
    citation=_cite("AC-6"),
)
...
_AU_2 = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="audit_event_type"),),
    deny_message=("NIST AU-2: Calls require an audit_event_type for the log."),
    citation=_cite("AU-2"),
)
...
_AU_3 = ComplianceRuleSpec(
    ...
    deny_message=(
        "NIST AU-3: Audit records require source_network_address and outcome metadata."
    ),
    citation=_cite("AU-3"),
)
...
_AU_12 = ComplianceRuleSpec(
    ...
    tags=frozenset({"nist_moderate"}),
    checks=(MetadataCheck(metadata_key="audit_correlation_id"),),
    deny_message=("NIST AU-12: Calls require an audit_correlation_id."),
...
_IA_5 = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="issuance_ticket_id"),),
    deny_message=("NIST IA-5: Authenticator issuance requires an issuance_ticket_id."),
    citation=_cite("IA-5"),
)
...
_SC_7 = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="boundary_gateway_id"),),
    deny_message=("NIST SC-7: External communications require a boundary_gateway_id."),
    citation=_cite("SC-7"),
)
...
_SC_8 = ComplianceRuleSpec(
    ...
    deny_message=(
        "NIST SC-8: Data in transit requires TLS 1.2 or higher / mTLS / IPsec."
    ),
    citation=_cite("SC-8"),
)
...
_SC_12 = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="key_management_system_id"),),
    deny_message=(
        "NIST SC-12: Cryptographic key operations require a key_management_system_id."
    ),
...
_SC_13 = ComplianceRuleSpec(
    ...
    checks=(
        MetadataCheck(
            ...
            allowed_values=frozenset({"fips_140_2", "fips_140_3", "cnsa_suite_2_0"}),
        ),
    ),
    deny_message=(
...
_SC_28 = ComplianceRuleSpec(
    ...
    deny_message=(
        "NIST SC-28: Data at rest requires an at_rest_protection attestation."
    ),
...
_SI_4 = ComplianceRuleSpec(
    ...
    tags=frozenset({"nist_moderate"}),
    checks=(MetadataCheck(metadata_key="monitoring_sensor_id"),),
    deny_message=("NIST SI-4: Monitored calls require a monitoring_sensor_id."),
    citation=_cite("SI-4"),
)
...
_SI_7 = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="integrity_attestation_id"),),
    deny_message=(
