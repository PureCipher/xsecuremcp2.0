    ...
    checks=(
        MetadataCheck(
            ...
            allowed_values=frozenset(
                {"deleted", "cryptographically_rendered_unrecoverable"}
            ),
        ),
    ),
    deny_message=(
...
_REQ_3_5_KEY_MGMT = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="kek_id"),),
    deny_message=(
        "PCI DSS 3.5: CHD encryption requires a kek_id (key-encrypting-key identifier)."
    ),
...
_REQ_4_2_ENCRYPT_TRANSMISSION = ComplianceRuleSpec(
    ...
    deny_message=("PCI DSS 4.2.1: CHD transmission requires TLS 1.2 or higher."),
...
_REQ_8_2_UNIQUE_ID = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="user_id"),),
    deny_message=("PCI DSS 8.2.1: Access requires a unique user_id."),
...
_REQ_8_4_MFA = ComplianceRuleSpec(
    ...
    deny_message=(
        "PCI DSS 8.4.2: Admin access to the CDE requires MFA (mfa_state=passed)."
    ),
...
_REQ_10_2_AUDIT = ComplianceRuleSpec(
    ...
    tags=frozenset({"cardholder_data", "pci_scoped"}),
    checks=(MetadataCheck(metadata_key="audit_event_id"),),
    deny_message=(
        "PCI DSS 10.2: CHD access requires an audit_event_id for log correlation."
    ),
...
_REQ_10_7_INCIDENT_DETECT = ComplianceRuleSpec(
    ...
    checks=(MetadataCheck(metadata_key="failure_ticket"),),
    deny_message=(
