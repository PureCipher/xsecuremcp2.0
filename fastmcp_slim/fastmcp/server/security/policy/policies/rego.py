...
def parse_rego(source: str) -> _Module:
    ...
    while i < len(lines):
        ...
        body_exprs = tuple(_parse_expression(ln) for ln in body_lines if ln.strip())
        rules.append(
            _Rule(
...
def _collect_body(lines: list[str], start: int) -> tuple[list[str], int]:
    ...
    while j < len(lines):
        ...
        body.append(line)
        j += 1
    raise RegoParseError(f"Rego rule starting at line {start + 1} has no closing '}}'")
...
def _parse_atom(text: str) -> Any:
    ...
    if text.startswith("{") and text.endswith("}"):
        ...
        items: list[Any] = []
        if inner.strip():
            items.extend(_parse_atom(piece) for piece in _split_top_level(inner, ","))
    ...
    if text.startswith('"') and text.endswith('"'):
        return _Literal(bytes(text[1:-1], "utf-8").decode("unicode_escape"))
...
def _parse_call(text: str) -> _Call:
    ...
    args: list[Any] = []
    if raw_args.strip():
        args.extend(_parse_atom(piece) for piece in _split_top_level(raw_args, ","))
