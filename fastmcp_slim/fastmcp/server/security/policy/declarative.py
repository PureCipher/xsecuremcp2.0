...
def _build_pci_request(config: dict[str, Any]) -> PciRequestPolicy:
    base = _build_zero_trust(
        {**config, "policy_id": config.get("policy_id", "pci-dss-request-validation")}
    )
...
def _build_hipaa_request(config: dict[str, Any]) -> HipaaRequestPolicy:
    base = _build_zero_trust(
        {**config, "policy_id": config.get("policy_id", "hipaa-request-validation")}
    )
...
def _register_builtins() -> None:
    ...
    builtins = [
        PolicyTypeDescriptor(
            type_key="pci_request",
            factory=_build_pci_request,
            display_name="PCI DSS Request Validation",
            category="compliance",
            description="Operation-aware account-data safeguards with trusted evidence.",
            field_specs={
                "grants": {
                    "label": "Exact grants",
                    "type": "json_list",
        ...
        PolicyTypeDescriptor(
            type_key="hipaa_request",
            factory=_build_hipaa_request,
            display_name="HIPAA Request Validation",
            category="compliance",
            description="Patient-specific PHI safeguards with trusted evidence.",
            field_specs={
                "grants": {
                    "label": "Exact grants",
                    "type": "json_list",
