...
_BUSINESS_ASSOCIATE = ComplianceRuleSpec(
    ...
    checks=(
        MetadataCheck(metadata_key="baa_reference"),
    ),
    deny_message=(
...
_RIGHT_OF_ACCESS = ComplianceRuleSpec(
    ...
    checks=(
        MetadataCheck(metadata_key="access_request_id"),
    ),
    deny_message=(
...
_UNIQUE_USER_IDENTIFICATION = ComplianceRuleSpec(
    ...
    tags=frozenset({"phi", "health_data", "hipaa_regulated"}),
    checks=(
        MetadataCheck(metadata_key="user_identifier"),
    ),
    deny_message=(
...
_AUDIT_CONTROLS = ComplianceRuleSpec(
    ...
    checks=(
        MetadataCheck(metadata_key="audit_sink"),
    ),
    deny_message=(
...
_INTEGRITY = ComplianceRuleSpec(
    ...
    checks=(
        MetadataCheck(
            ...
            allowed_values=frozenset({"sha256_digest", "sha512_digest", "hmac", "signature"}),
        ),
    ),
    deny_message=(
