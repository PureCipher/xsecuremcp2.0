...
def _parse_entity_literal(text: str) -> str:
    ...
    if "::" not in trimmed:
        raise CedarParseError(
            f"Entity literal must be Type::id, got {trimmed!r}"
        )
...
def _parse_expression(text: str) -> Any:
    ...
    if parser.pos < len(tokens):
        raise CedarParseError(
            f"Unexpected token {tokens[parser.pos]!r} in expression"
        )
    return expr
...
def _eval_expr(expr: Any, ctx: PolicyEvaluationContext) -> Any:
    ...
    if isinstance(expr, _ExprBinOp):
        if expr.op == "&&":
            return bool(_eval_expr(expr.lhs, ctx)) and bool(
                _eval_expr(expr.rhs, ctx)
            )
        if expr.op == "||":
            return bool(_eval_expr(expr.lhs, ctx)) or bool(
                _eval_expr(expr.rhs, ctx)
            )
        ...
        for permit in fired_permits:
            if "@require_approval" in permit.raw:
                ...
                return PolicyResult(
                    ...
                    reason=(
                        f"Cedar permit requires approval: "
                        f"{_summarize(permit)}"
                    ),
                    policy_id=self._policy_id,
                )
